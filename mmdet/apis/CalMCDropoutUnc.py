...
def MCDropout_uncertainty(cfg, model, data_loader, **kwargs):
    uncertainties = MCDropout_MI(model, data_loader, **kwargs)
...
def MCDropout_MI(model, data_loader, n = 25, **kwargs):
    model.eval()
    uncertainties, latencies = [], []
    dataset = data_loader.dataset
    prog_bar = mmcv.ProgressBar(len(dataset))
    ...
    with torch.no_grad():
        for i, data in enumerate(data_loader):
            ...
            for _ in range(n):
                start = time.time()
                Out = model(return_loss=False, rescale=True, isEval=True, justOut=True, batchIdx=i, **data, **kwargs)
                end = time.time()
                latencies.append(end - start)
                outList.append(Out)
            batch_size = len(Out[0])
            for _ in range(batch_size):
                prog_bar.update()
            UncOuts = ComputeMCDropoutMI(*outList, nCls=20)
            uncertainties.extend(UncOuts)

    torch.cuda.empty_cache()
    uncertainties = torch.tensor(uncertainties)
    print(f'latency is {torch.tensor(latencies).mean()}')
    return uncertainties
...
def ComputeMCDropoutMI(*MCOuts, nCls = 20):
    n, S, B = len(MCOuts), len(MCOuts[0]), len(MCOuts[0][0])
    buffer = torch.zeros(B,S) # Modify It!
    for sIdx, scaleOuts in enumerate(zip(*MCOuts)):
        for bIdx, bOuts in enumerate(zip(*scaleOuts)):
            stkOuts = torch.stack(bOuts) # [25,180,76,116]
            sigOuts = F.sigmoid(stkOuts).permute([0,2,3,1])
            rshOuts = sigOuts.reshape(*sigOuts.size()[:-1],-1,nCls)
            preds = rshOuts.reshape(n,-1,nCls)
            avg = preds.mean(dim=0)
            total = (-avg * avg.log()).sum(dim=1)
            ent = (-preds * preds.log()).sum(dim=-1)
            aleatoric = ent.mean(dim=0)
