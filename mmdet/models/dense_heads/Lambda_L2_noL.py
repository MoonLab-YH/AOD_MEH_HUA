...
class Lambda_L2Net_NoL(L_AnchorHead):
    def __init__(self,
                 num_classes,
                 in_channels,
                 stacked_convs=4,
        ...
        self.isTrainD = False
        super(Lambda_L2Net_NoL, self).__init__(num_classes, in_channels, anchor_generator=anchor_generator, init_cfg=init_cfg, **kwargs)
    ...
    def ComputeAvgUnc(self, mlvl_cls_scores, L_scores):
        ...
        for sIdx, slvl_scores in enumerate(mlvl_cls_scores):
            for iIdx, simg_scores in enumerate(slvl_scores):
                ...
                if not True in FGIdx:
                    continue
                fil_dist = Dirichlet(slvl_alphas[FGIdx])
