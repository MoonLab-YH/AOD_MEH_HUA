...
class WeakSTNet(D_AnchorHead):
    ...
    def loss_single(self, cls_score, bbox_pred, anchors, labels, label_weights,
        ...
        prob_threshold, neg_threshold, AllNeg = 0.4, 0.1, True
        ...
        if kwargs['Labeled'] and (not kwargs['Pseudo']):
            ...
            for Bbox_pred, Cls_score, Label_conf_weight, anchor, Label_weight in zipped:
                ...
                easy_inds = (-JSD[pseudo_cls_weight]).topk(int(Cls_score.size(0) * neg_threshold))[1]
                all_inds = torch.cat((easy_inds, pos_inds)).unique()
                if AllNeg and len(pos_inds) == 0:
                    all_inds = torch.ones_like(pseudo_cls_weight)
                loss_cls += self.loss_cls(Cls_score[all_inds], pseudo_labels[all_inds], pseudo_cls_weight[all_inds], avg_factor=num_total_samples).sum()
                # # With Real GT Labels...
                ...
                if f_cnt >= 2000:
                    ...
                    if visOn:
                        ...
                        savedir = f'visualization_WeakST,p({prob_threshold}),q({neg_threshold})'
