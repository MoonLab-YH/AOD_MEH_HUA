...
class A2LparamgenNet(L_AnchorHead):
    def __init__(self,
                 num_classes,
                 in_channels,
                 stacked_convs=4,
        ...
        self.isTrainD = False
        super(A2LparamgenNet, self).__init__(
            num_classes,
            in_channels,
            anchor_generator=anchor_generator,
            init_cfg=init_cfg,
            **kwargs)
        self.L_names = ['retina_L', 'L_convs', 'retina_gen_mul', 'retina_gen_add']
    ...
    def _init_layers(self):
        ...
        for i in range(self.stacked_convs):
            ...
            self.reg_convs.append(ConvModule(chn, self.feat_channels, 3, stride=1, padding=1,
                                             conv_cfg=self.conv_cfg, norm_cfg=self.norm_cfg))
            self.L_convs.append(ConvModule(chn, chn, 3, stride=1, padding=1,
                                           conv_cfg=self.conv_cfg, norm_cfg=self.norm_cfg))
        ...
        self.retina_L = nn.Conv2d(chn, self.num_anchors, 3, padding=1)
        self.retina_gen_mul = nn.Sequential(nn.Conv2d(180, 220, 3, 1, 1), nn.BatchNorm2d(220), nn.ReLU(),
                                         nn.Conv2d(220, 256, 3, 1, 1), nn.BatchNorm2d(256), nn.ReLU())
        self.retina_gen_add = nn.Sequential(nn.Conv2d(180, 220, 3, 1, 1), nn.BatchNorm2d(220), nn.ReLU(),
                                            nn.Conv2d(220, 256, 3, 1, 1), nn.BatchNorm2d(256), nn.ReLU())
    ...
    def forward_L(self, feats, head_out, **kwargs):
        return multi_apply(self.forward_single_L, feats, head_out[1])[0]
    ...
    def forward_single_L(self, x, cls_score):
        L_gen_mul = self.retina_gen_mul(cls_score.detach())
        L_gen_add = self.retina_gen_add(cls_score.detach())
        L_feat = x * L_gen_mul + L_gen_add
    ...
    def loss_single_L(self, L_score, loss, label_weights, **kwargs):
        label_weights = label_weights.reshape(-1)
        L_score = L_score.permute(0, 2, 3, 1).reshape(-1) + 1e-9
        loss_L = torch.abs(L_score - loss)
        # loss_L = (loss_L * label_weights).mean()
        loss_L = (loss_L * label_weights).mean() * 2
        return loss_L, 0
    ...
    def simple_test(self, feats, img_metas, rescale=False, **kwargs):
        ...
        L_scores = self.forward_L(feats, head_out=[None, outs[0]])
    ...
    def SaveUnc(self, AggedUnc, **kwargs):
        for iIdx, aggedUnc in zip(range(len(AggedUnc)), AggedUnc):
            visualize(kwargs['_data'][iIdx], f'visualization/{iIdx}img__{aggedUnc}.jpg')
        ...
        for sIdx, slvl_scores in enumerate(mlvl_cls_scores):
            for iIdx, simg_scores in enumerate(slvl_scores):
                ...
                if not True in FGIdx:
                    ...
                    continue
                visualize(kwargs['_data'][iIdx], f'visualization/{iIdx}img.jpg')
                maxAlpha = slvl_alphas.max(dim=1)[0]
                maxmaxAlpha = maxAlpha.reshape(H, W, 9).max(dim=2)[0]
                visualize(maxmaxAlpha, f'visualization/{iIdx}img_{sIdx}scale_Alpha.jpg', size=(oriH, oriW), heatmap=True)
                l_scores = mlvl_l_scores[sIdx][iIdx].permute(1, 2, 0).reshape(-1, 1)
                l_scores = l_scores.mean() / (l_scores + 1e-9) * 25
                slvl_L_alphas = slvl_alphas * l_scores
                l_vis = l_scores.reshape(H, W, 9).sum(dim=-1)
                visualize(l_vis, f'visualization/{iIdx}img_{sIdx}scale__Lambda.jpg',
                          size=(oriH, oriW), heatmap=True)
                for alphas, _type in zip([slvl_alphas, slvl_L_alphas], ['_NOL_','_L_']):
                    dist = Dirichlet(alphas)
                    samples = dist.sample(torch.tensor([500]))  # [sample_size, num_bbox, n_way(20)]
                    ...
                    visualize(avg_epistemic, f'visualization/{iIdx}img_{sIdx}scale{_type}avgEpi.jpg',
                              size=(oriH, oriW), heatmap=True)
                    max_epistemic = epistemic.reshape(H, W, 9).max(dim=2)[0]  # [H,W]
                    visualize(max_epistemic, f'visualization/{iIdx}img_{sIdx}scale{_type}maxEpi.jpg',
                              size=(oriH, oriW), heatmap=True)
