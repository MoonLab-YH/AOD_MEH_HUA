...
class A2LdowncatNet(L_AnchorHead):
    def __init__(self,
                 num_classes,
                 in_channels,
                 stacked_convs=4,
        ...
        self.isTrainD = False
        super(A2LdowncatNet, self).__init__(
            num_classes,
            in_channels,
            anchor_generator=anchor_generator,
            init_cfg=init_cfg,
            **kwargs)
        self.L_names = ['retina_L', 'L_convs', 'retina_down']
    ...
    def _init_layers(self):
        ...
        for i in range(self.stacked_convs):
            ...
            self.reg_convs.append(ConvModule(chn, self.feat_channels, 3, stride=1, padding=1,
                                             conv_cfg=self.conv_cfg, norm_cfg=self.norm_cfg))
            self.L_convs.append(ConvModule(chn_L, chn_L, 3, stride=1, padding=1,
                                           conv_cfg=self.conv_cfg, norm_cfg=self.norm_cfg))
        ...
        self.retina_L = nn.Conv2d(chn_L, self.num_anchors, 3, padding=1)
        self.retina_down = nn.Sequential(nn.Conv2d(180, 120, 3, 1, 1), nn.BatchNorm2d(120), nn.ReLU(),
                                         nn.Conv2d(120, 64, 3, 1, 1), nn.BatchNorm2d(64), nn.ReLU())
    ...
    def forward_single_L(self, x, cls_score):
        downed = self.retina_down(cls_score.detach())
        L_feat = torch.cat((x, downed), dim=1)
