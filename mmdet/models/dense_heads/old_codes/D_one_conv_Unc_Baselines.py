...
class JustOneConv_UncBase(D_AnchorHead):
    def __init__(self,
                 num_classes,
                 in_channels,
                 stacked_convs=4,
        ...
        self.isTrainD = False
        super(JustOneConv_UncBase, self).__init__(
            num_classes,
            in_channels,
    ...
    def loss_single(self, cls_score, bbox_pred, anchors, labels, label_weights,
        ...
        if kwargs['Labeled'] and (not kwargs['Pseudo']):
            ...
            for Bbox_pred, Cls_score, Label_conf_weight, anchor, Label_weight in zipped:
                ...
                loss_cls += self.loss_cls(Cls_score, pseudo_labels.detach(), pseudo_cls_weight, avg_factor=num_total_samples).sum()
    ...
    def _get_bboxes(self, mlvl_cls_scores, mlvl_bbox_preds, mlvl_anchors, img_shapes, scale_factors,
        ...
        if kwargs['isUnc']:
            if kwargs['uPool2'] == 'max':
                AggedUnc = self.ComputeScaleMaxUnc(mlvl_cls_scores)
            else:
    ...
    def ComputeScaleUnc(self, mlvl_cls_scores):
        ...
        for sIdx, slvl_scores in enumerate(mlvl_cls_scores):
            for iIdx, simg_scores in enumerate(slvl_scores):
                ...
                samples = fil_dist.sample(torch.tensor([100]))  # [sample_size, num_bbox, n_way(20)]
                avg = samples.mean(dim=0)
                total = (-avg * avg.log()).sum(dim=1)
                ent = (-samples * samples.log()).sum(dim=-1)
                aleatoric = ent.mean(dim=0)
    ...
    def ComputeScaleMaxUnc(self, mlvl_cls_scores, uncType = 'Tot'):
        S = len(mlvl_cls_scores)
        B, _, oriH, oriW = mlvl_cls_scores[0].shape
        output = [[[] for _ in range(S)] for _ in range(B)]
        for sIdx, slvl_scores in enumerate(mlvl_cls_scores):
            for iIdx, simg_scores in enumerate(slvl_scores):
                simg_scores = simg_scores.permute(1, 2, 0).reshape(-1, self.cls_out_channels)
                slvl_alphas = simg_scores.softmax(dim=1)  # [ N, 20]
                fil_dist = Dirichlet(slvl_alphas)
                samples = fil_dist.sample(torch.tensor([100]))  # [sample_size, num_bbox, n_way(20)]
                avg = samples.mean(dim=0)
                total = (-avg * avg.log()).sum(dim=1)
                ent = (-samples * samples.log()).sum(dim=-1)
                ...
                epistemic = total - aleatoric
                if uncType == 'Epi':
                    output[iIdx][sIdx] = epistemic.max()
                elif uncType == 'Tot':
                    output[iIdx][sIdx] = total.max()
        maxUnc = [torch.stack(i).max() for i in output]
        return maxUnc
