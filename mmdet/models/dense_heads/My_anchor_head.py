...
class MyAnchorHead(BaseDenseHead, BBoxTestMixin):
    ...
    def loss(self, cls_scores, bbox_preds, gt_bboxes, gt_labels, img_metas, gt_bboxes_ignore=None, **kwargs):
        ...
        all_cls_scores = torch.cat([s.permute(0, 2, 3, 1).reshape(num_images, -1, self.cls_out_channels)
                                    for s in cls_scores], 1)
        ...
        head_info = ['cls_scores', 'bbox_preds', 'all_anchor_list(N)', 'labels_list', 'label_weights_list',
                     'bbox_targets_list', 'bbox_weights_list', 'num_total_samples(N)']
        ...
        return dict(loss_cls=losses_cls, loss_bbox=losses_bbox), head_out
    ...
    def _get_bboxes(self, mlvl_cls_scores, mlvl_bbox_preds, mlvl_anchors, img_shapes, scale_factors,
        ...
        if with_nms:
            ...
            for (mlvl_bboxes, mlvl_scores) in zip(batch_mlvl_bboxes, batch_mlvl_scores):
                ...
                # ShowNMSed(det_bbox, det_label, idces, 0, show_thr=0.3, **kwargs)
        else:
