...
class SSDAnchorHead(BaseDenseHead, BBoxTestMixin):
    ...
    #     losses_cls, losses_bbox = multi_apply(self.loss_single, cls_scores, bbox_preds, all_anchor_list, labels_list,
    #         label_weights_list, bbox_targets_list, bbox_weights_list, num_total_samples=num_total_samples)
    ...
    def loss(self, cls_scores, bbox_preds, gt_bboxes, gt_labels, img_metas, gt_bboxes_ignore=None, **kwargs):
        ...
        cls_reg_targets = self.get_targets(anchor_list, valid_flag_list, gt_bboxes, img_metas,  # unmap_outputs=False,
        ...
        # all_anchors = []
        # for i in range(num_images): all_anchors.append(torch.cat(anchor_list[i]))
        ...
        head_out = (head_info, cls_scores, bbox_preds, all_anchor_list, labels_list, label_weights_list
                    , bbox_targets_list, bbox_weights_list, num_total_samples)
        # losses_cls, losses_bbox = multi_apply(self.loss_single, all_cls_scores, all_bbox_preds, all_anchors, all_labels,
        #                                       all_label_weights, all_bbox_targets, all_bbox_weights, [0, 1, 2, 3, 4],
        #                                       num_total_samples=num_total_pos, featmap_sizes=featmap_sizes, **kwargs)
        losses_cls, losses_bbox, losses_noR = multi_apply(self.loss_single, cls_scores, bbox_preds, all_anchor_list, labels_list,
    ...
    def loss_L(self, L_scores, head_out, losses, **kwargs):
        label_weights_list = head_out[5]
        bbox_weights_list = head_out[7]
        losses_L, _ = multi_apply(self.loss_single_L, L_scores, losses, label_weights_list, bbox_weights_list, **kwargs)
        return dict(loss_L=losses_L)
