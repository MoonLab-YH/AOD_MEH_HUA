...
class SelfSSL_SingleStageDetector(SelfSSLBaseDetector):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(SelfSSL_SingleStageDetector, self).__init__(init_cfg)
        if pretrained:
    ...
    def forward_train(self,
                      ...
                      **kwargs):
        super(SelfSSL_SingleStageDetector, self).forward_train(img, img_metas)
        kwargs['img'] = img
        x = self.extract_feat(img)
        x_flip = self.extract_feat(kwargs['flipImg']) if 'flipImg' in kwargs else None
        # losses = self.bbox_head.forward_train(x, img_metas, gt_bboxes, gt_labels, gt_bboxes_ignore)
        losses = self.bbox_head.forward_train_SelfSSL(
            x, img_metas, gt_bboxes, gt_labels, gt_bboxes_ignore, x_flip = x_flip, **kwargs)
        return losses
