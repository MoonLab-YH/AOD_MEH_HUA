...
class MyEpochBasedRunnerLSSD(BaseRunner):
    def run_iter(self, data_batch, train_mode, **kwargs):
        if self.batch_processor is not None:
            outputs = self.batch_processor(self.model, data_batch, train_mode=train_mode, **kwargs)
        elif train_mode:
            loss, head_out, feat_out, prev_loss = self.model.train_step(data_batch, **kwargs)
            self.optimizer.zero_grad()
            loss['loss'].backward()
            self.optimizer.step()
            # From here, newely added..
            loss_L = self.model.module.train_step_L(prev_loss, head_out, feat_out, _data = data_batch, **kwargs)
